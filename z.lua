...
local function flingAll()
 ...
 local Toggle = Tab:CreateToggle({
    Name = "HR Notifier",
    CurrentValue = false,
    Flag = "HRNotifierToggle",
    Callback = function(Value)
        hrNotifierEnabled = Value
        if hrNotifierEnabled then
            sendNotification("NOTIFICATION", "HR Notifier is ENABLED")
            -- Listen for new players joining
            game.Players.PlayerAdded:Connect(checkAndNotifyHR)
        else
            sendNotification("NOTIFICATION", "HR Notifier is DISABLED")
        end
    end
})
